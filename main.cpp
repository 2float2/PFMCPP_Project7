...
Purpose:  This project will teach you how to read existing code someone else has
already written, and to get it working again.  Consider this project the
...
revert to a working version easily if needed. it's very easy to mess this task
up.
 ...
 2) Assertions have been added with text explaining the things you need to
implement. after you implement them, remove the 'assert(false);

 3) add the appropriate #include statements.  Remove any unnecessary #include
statements

 4) you're going to need to implement the pure virtual functions in
Character-derived classes and Item classes
 ...
 6) Each character type gets some Helpful Items and Defensive Items when they're
constructed Use the functions in Utility.h to give each Character (excluding
Dragon, because dragons don't carry items) a RANDOM NUMBER of helpful and
defensive items: helpfulItems = makeHelpfulItems(2); defensiveItems =
makeDefensiveItems(1);

 7) Dragon Slayers are allowed 1 special item: an AttackItem
    create a new Item-derived type that boosts the attack damage called
AttackItem add an instance of it to the DragonSlayer class make the
DragonSlayer::attack use it when the dragonSlayer attacks a dragon.

 8) Clear the warnings as best you can.
         for the 'vtables' warnings coming from a header file, move any virtual
function implementations to the complementary cpp file. for 'shadow' warnings,
rename the offending variable. casting is introduced in the next video and
project, so ignore any 'implicit conversion' warnings.

 9) When you compare your output with the expected output, it is fine if the
items counts don't match exactly.   It's not important if your DragonSlayer has
5 helpful items, but the expected output shows 3.

 10) correct any unnecessary keywords related to access specifiers used in the
UDTs.
